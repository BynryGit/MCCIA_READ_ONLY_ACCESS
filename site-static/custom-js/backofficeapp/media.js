function add_new_row(){	
        $("#promocode_body").append('<tr >'+
            ...
            '<td class="has-success">'+'<input type="text" name="max" class="form-control" onchange="change_min(this)">'+'</td>'+
...
$("#registr_date").change(function () {
		$(registr_date_error).css("display", "none");
});
...
$(document).ready(function(){
    get_hall_location();
     $.ajax({
             ...
             data: {'event_detail_id': $("#hidden_event_details").val()},
             success: function (response) {   
               if (response.success == 'true') {                              
                 $('#summernote_1').code(response.event_description_indetails);
               }                    
             },
...
function get_hall_name() {
 ...
 function get_hall_location() {
        select_hall_id = $('#select_hall_id :selected').val();
        if (select_hall_id) {
            $.ajax({
                type: 'GET',
...
function get_contact_person() {
 ...
 function upload_event_banner() {
   ...
   $(edit_event_banner).css("display", "none");
   $("#hidden_event_banner").val(1);
  ...
  function upload_sponsor_banner() {
 	...
 	$(edit_sponsor_banner).css("display", "none");
 	$("#hidden_sponsor_banner").val(1);
 ...
 $("#preview-continue").click(function(event) { 
  	...
  	if (validateData()) {
  		  ...
  		  hidden_from_date = $("#hidden_from_date").val()						
    	  start_date=hidden_from_date.split("-"); //04 July 2018 - 06:08 Split by '-'
    	  ...
    	  hidden_from_dt = new Date(new_start_date[2],month_list.indexOf(new_start_date[1]),new_start_date[0],new_start_time[0],new_start_time[1])
    	  
    	  new_from_date = $("#from_date").val()
    	  from_date=new_from_date.split("-"); //04 July 2018 - 06:08 Split by '-'
    	  new_from_date1=from_date[0].split(" ");// we get => [04,July,2018]
    	  new_from_time1=from_date[1].split(":");// we get => [06,08]	    	   
    	  // new Date(Year, Month, Date, Hr, Min, Sec);	    	  
    	  new_from_dt = new Date(new_from_date1[2],month_list.indexOf(new_from_date1[1]),new_from_date1[0],new_from_time1[0],new_from_time1[1])
    	  
    	  var today_date = new Date();
    	 
    	  if (hidden_from_dt > today_date) {
					if (new_from_dt > today_date) {
							   ...
							   $("#main_div").css("display", "none");
								$("#preview_div").css("display", "block");	
							 	...
							 	$.ajax({
						                ...
						                data: {'event_details_id':$("#hidden_event_details").val()},
						                success: function (response) {   
						                  if (response.success == 'true') {                              
						                    var preview_event_banner = document.getElementById("preview_event_banner");
						    					  ...
						    					  var preview_sponsor_banner = document.getElementById("preview_sponsor_banner");
						    					  preview_sponsor_banner.src = response.sponsor_docs_address;        
						                  }                    
						                },
						                error: function (response) {
						                    alert("Error!");
						...
						toastr.error("Event from date is smaller than today date");
						return false;
					}       			
	    	  }
	    else {
	    	toastr.error("Event already started ");	    
	    	return false;
	    }	  	      		
    }
});
 ...
 $("#submit_event").click(function(event) { 	 			
        ...
        event.preventDefault(); 
    	  ...
    	  formData.append("event_details_id",$('#hidden_event_details').val());
    	  
        $.ajax({            
             type: "POST",
       		 url: "/backofficeapp/update-event/",
       		 data : formData,
				 cache: false,
		       processData: false,
		 		 contentType: false, 
             success: function(response) {
	                ...
	                if (response.success == "true") {
	                    //
	                    bootbox.alert("<span class='center-block text-center'>Event updated successfully</span>",function(){
...
function validateData() {
    if (CheckCommittee("#select_committee")&CheckPerson1("#select_contact1")&CheckPriority("#select_priority")&CheckEventType("#select_event_type")
        ...
        &CheckForWhom("#for_whom")&CheckOrganisedBy("#organised_by")&checkMemberCharges("#member_charges")&checkNonMemberCharges("#non_member_charges")
        &checkifOther("#select_other")&checkifEarlyBird()&checkEventBanner("#event_banner")&checkSponsorBanner("#attachments")
...
function checkHallName(select_hall_id) {
    if ($(select_hall_id).val() != '' && $(select_hall_id).val() != null) {
        $("#select_hall_locationDiv").addClass("has-success").removeClass("has-error");
        return true;
    } else {
...
function checkifEarlyBird() {
	if ($('#checkbox1_1').prop("checked")) {
		    ...
		    early_date=$("#early_date").val()
			 if (early_date == '' || early_date ==undefined){
...
function checkSponsorBanner(attachments) {
	return true;
	if ($('#attachments').val()=='') {
...
Dropzone.options.myAwesomeDropzone3 = {
    ...
    init: function() {
        ...
        doc_length = parseInt($('#image_countt').val())
        if (doc_length > 0){
            var productImages = $('#imagesurl').val(); // getting the list from hidden field
            var image_ids = $('#image_listt').val(); // getting the list from hidden field
            productImages = productImages.split(','); // split the string so that it form javascript array
            image_ids = image_ids.split(','); // split the string so that it form javascript array
            for (var i = 0; i < productImages.length; i++) {
                ...
                this.emit("thumbnail",mockFile, productImages[i]);
                this.files.push(mockFile);
                temp_image_files.push(mockFile);
                }
        }
...
//            $.ajax({
//                url: "/nscapp/edit-remove-consumer-docs/?image_id=" + deleting_image_id,
