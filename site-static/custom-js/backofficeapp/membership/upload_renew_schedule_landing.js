$(document).ready(function () {
  request_schedule_upload_table();
});


function clear_schedule_file() {
    $("#upload_schedule_file").val('');
}


function upload_schedule_file(){
var flag=true

if ($('#upload_schedule_file').val() == ''){
    flag=false
    $("#error_schedule_file").addClass('has-error')
    }

    var input = document.getElementById("upload_schedule_file");
    schedule_file = input.files[0];
...
if (flag){
    $.ajax({
        type:'POST',
        url:'/backofficeapp/upload-schedule-file/',
        data: formData,
        cache: false,
        processData: false,
        contentType: false,
        success: function(response) {
            if (response.success == "true") {
                alert("Associate Schedule Uploaded Successfully")
                window.location.href = "/backofficeapp/upload-renew-schedule-landing";
                clear_schedule_file()
            }
        },
...
function request_schedule_upload_table(){
    var table = $('#upload_schedule_table');
    var oTable = table.dataTable({
        "processing": true,
        "serverSide": true,
        "destroy": true,
        "ajax": '/backofficeapp/get-schedule-datatable/',
        "searching": true,
        "Filter": true,
        "ordering": false,
        "paging": true,
