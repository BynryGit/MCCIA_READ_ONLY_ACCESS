...
$("#online_pay_button_div").css('display','none');
...
$(document).ready(function(){
       ...
       booking_from_date = $("#from_booking_date").val()
       to_booking_date = $("#to_booking_date").val()
 	   load_hall_reg_details(select_location,select_hall,select_payment,start_date,end_date,booking_from_date,to_booking_date);
});
...
function clear_filter() {
       ...
       // $("#select_hall").val("all").trigger('change');
       // $("#select_payment").val("").trigger('change');
       // $("#from_date").val("");
       // $("#to_date").val("");
...
var date_input=$('.dateField'); //our date input has the name "date"
 ...
 function load_hall_reg_details(select_location,select_hall,select_payment,start_date,end_date,booking_from_date,to_booking_date) {    
 var table = $('#hallregtable');
 var oTable = table.dataTable({
        //"processing": true,
        "serverSide": true,
        "destroy": true,
        "ajax": "/backofficeapp/get-hall-regs-datatable/?select_location="+select_location+'&select_hall='+select_hall+'&select_payment='+select_payment+"&start_date="+start_date+"&end_date="+end_date+"&booking_start_date="+booking_from_date+"&to_end_date="+to_booking_date,
        "searching": true,
        "paging": true,
        "columnDefs": [
...
function open_mail_request(id){
       window.location.href ="/backofficeapp/send-booking-proforma-mail-page-render/?booking_id="+id
}
...
function send_proforma_invoice_mail(){
     ...
     discount_got = $("#discount_got").val();
    $.ajax({
            type: "GET",
            url: '/backofficeapp/send-booking-proforma-mail/',
            data: {'booking_id': booking_id, 'final_total':final_total, 'discount_got':discount_got},
            success: function (response) {
                if(response.success=='true'){
                   alert('Mail Sent Successfully.');
                   window.location.href ="/backofficeapp/hall-booking-registration/"
                }
                else{
                    bootbox.alert('Sorry for inconvenience. An error occurred');
                }
...
function edit_proforma_invoice_mail(){
	booking_id = $("#booking_id").val();
	window.location.href ="/backofficeapp/edit-booking-proforma-mail-page-render/?booking_id="+booking_id
}


function update_proforma_invoice_mail(){
	booking_id = $("#booking_id").val();
	$.ajax({
            type: "GET",
            url: '/backofficeapp/update-booking-proforma-mail/',
            data: {'booking_id': booking_id,
            		'name' : $("#booking_name").val(),
            		'address' : $("#booking_address").val(),
            		'membership_no' :$("#booking_membership_no").val(),
            		'contact_person' : $("#contact_person").val(),
            		'mobile_no':$("#mobile_no").val(),
            		'tel_o' :$("#tel_o").val(),
            		'tel_r':$("#tel_r").val(),
            		'email' : $("#email").val(),
            		'facility_amount' : $("#facility_amount").val(),
            		'discount_got' : $("#discount_got").val(),
            ...
            success: function (response) {
                if(response.success=='true'){
                   ...
                   window.location.href ="/backofficeapp/send-booking-proforma-mail-page-render/?booking_id="+booking_id
                }
                else{
...
function cancel_proforma_invoice_mail(){
	window.location.href ="/backofficeapp/hall-booking-registration/"
}
...
//        else{
//            if(checkONLINEAmount_Submit("#online_amount_Submit")&checkTDS("#TDS_Submit")){
//                return true;
//            }
//            return false;
//        }
...
//function checkONLINEAmount_Submit(online_amount_Submit){
//    online_amount_Submit = $(online_amount_Submit).val()
//    TDS_amount = $("#TDS_Submit").val()
//    total_paying_amt = parseFloat(online_amount_Submit) + parseFloat(TDS_amount)
//
//    if(online_amount_Submit!='' & parseFloat(total_paying_amt) <= parseFloat($("#remain_amount").val())) {
//        $('#online_amount_Submit_error').css("display", "none");
//        return true;
//    }else{
//        $('#online_amount_Submit_error').css("display", "block");
//        $('#online_amount_Submit_error').text("Paying Amount should be equal or less then remaing amount");
//        return false;
//    }
//}
...
$('input[name=paymentType_Submit]').change(function(){
    ...
    if (paymentType == "ByCheque"){
        ...
        $("#Onlinepay_SubmitDiv").css('display','none');
        $("#offline_payment_detail").css('display','block');
        $("#online_pay_button_div").css('display','none');
            ...
            $('#cash_amount_Submit_error').css("display", "none");
            $('#Deposit_SubmitDiv').css("display", "none");
            $("#Onlinepay_SubmitDiv").css('display','block');
            $("#offline_payment_detail").css('display','none');
            $("#online_pay_button_div").css('display','block');
...
$("#submit_online_payment_detail").click(function(event){
	event.preventDefault();
//	if(validateDataSubmit()){
        var formData= new FormData();
	    formData.append("booking_id", $('#booking_id').val());
		formData.append("user_Payment_Type",$('input[name=paymentType_Submit]:checked').val());
		formData.append("online_amount",$('#online_amount_Submit').val());
		formData.append("TDS_amount",$('#TDS_Submit').val());
        $('#submit_online_payment_detail').attr("disabled", true);


          $.ajax({
                type: "POST",
                url: '/backofficeapp/send-link-for-online-payment/',
                data: formData,
                cache: false,
                processData: false,
                contentType: false,
                success: function (response) {
                    if(response.success=='true'){
                        $('#submit_online_payment_detail').attr("disabled", false);
                        bootbox.alert('Payment Link Send successfully');
                        $("#hall_booking_payment_modal").modal('hide');
    //                    $('#add_hall_booking_payment_form').trigger("reset");
                        filter();
                    }
